	...
	private void printQuakes() 
	{
		...
		int oceanQuakes = quakeMarkers.size();
	        for (Marker cm : countryMarkers) {
	        	...
	        	for(Marker cq : quakeMarkers) {
	        		 EarthquakeMarker em = (EarthquakeMarker)cq;
	        		if(em.isOnLand()) {
	        			
	        			 if(name.equals(em.getStringProperty("cm"))) {
	        				 quackeCounter++;
	        			 }
	        		        		}
	        	}
	        	if(quackeCounter>0) {
	        		oceanQuakes -= quackeCounter;
	        		System.out.println(name + " : " + quackeCounter);
